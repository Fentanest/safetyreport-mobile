...
- 이번 업데이트는 초기화 크롤링을 한 번 무조건 하므로 이전 DB 를 새 구조로 옮기지 않는다. `onUpgrade: _migrateLocalDatabase`·`backupBeforeUpgrade` 호출을
  ...
  새 빈 DB 에 감시목록·지오코딩 캐시만 옮겨 바꾼다. 예전에 직접 고친 값·중복 판단·메모와 신고는 옮기지 않고, 초기화 크롤링 화면이 이 사실과 백업 경로를 안내한다.
...
- GPT-6-Sol 검토 반영: 이전 DB 백업을 파일 복사 대신 `VACUUM INTO` 로 만든다(다른 연결 때문에 체크포인트가 끝나지 못하면 WAL 에만 있던 쓰기가 빠질 수 있었다),
  데모(심사용) 계정은 첫 로그인이 시드한 신고 때문에 초기화 화면이 나오지 않는다. 재검증 반영: 백업부터 파일 교체까지 옛 DB 쓰기 잠금을 잡아
  그 사이 쓰기가 백업·새 DB 어디에도 없이 사라지지 않게 했고, 이전 버전 데모 DB 를 비울 때 실제 계정의 커뮤니티 데이터셋을 회전하지 않는다.
...
- 테스트: `test/storage/migration_test.dart` 를 새 동작 기준으로 다시 씀(v9·v10·v14 백업 뒤 비우기, 되돌림, 새 설치), 복원·가져오기 거절, 새 테스트
  `test/community/rebuild_fresh_install_test.dart`, 초기화 화면 위젯 4건, WAL 백업·잠금·데모 제외. 대조 실험 11건. 전체 556 passed(3 skipped) 2회, analyze 0건,
