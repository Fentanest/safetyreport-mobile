...
>안전신문고에 신고한 민원의 처리 상태를 한 곳에서 보고, 변경되면 즉시 알림을 받는 개인용 Android 앱입니다.<br>
>공식 앱의 파편화된 정보를 한 눈에 볼 수 있게 모아주고 통계를 산출해 줍니다.<br>
>서버와 연동하는 Client방식과 단독으로 작동하는 Standalone방식 두 가지 사용 방식 중 하나를 선택할 수 있습니다.

<p align="center">
  <img src="example.png" alt="앱 화면 예시" />
</p>
...
4. 끝. 신고 내역, 통계, 감시 목록, 중복 신고 관리, 처리상태 변경 알림 사용 가능
...
| 중복 신고 관리 (검토 필요/중복 확정/중복 아님, 대표건 관리) | ✅ | ✅ |
| 개별 신고 데이터 직접 수정 | ✅ | ✅ |
| 처리상태 변경 자동 감지 + 알림 | ✅ | ✅ |
| 중복 신고 변경 알림 + 상세 보기 | ✅ | ✅ |
| 기관별 / 담당자별 통계 + 위반법규 필터 | ✅ | ✅ |
...
| 탭 | 내용 |
|----|------|
| 대시보드 | 처리 요약, 최근 답변, 감시목록, 하단 신고현황(Sunwi) |
| 신고내역 | 교통 / 주정차 / 기타 / 중복차량 |
| 신고관리 | 감시 목록 / 중복 신고 / 데이터 수정 |
| 통계 | 연도 × 카테고리 × 유형별 처리 통계, 위반법규 필터 |
| 알림 | 변경 알림 히스토리 (최대 200개) |
| 파일 | 로컬 / 서버 파일 브라우저 (Excel 포함) |
| 동기화 / 크롤링 | Standalone 동기화 / Client 크롤링 제어 |

- `신고현황`은 별도 하단 탭이 아니라 **대시보드 최하단 섹션**으로 표시됩니다.
- 중복 신고 알림을 누르면 대표건, child 목록, 상태, 비고까지 포함한 상세 시트를 바로 열 수 있습니다.
...
**Q. 백업 파일은 서로 호환되나요?**  
A. 가능합니다. 모바일 Standalone DB는 서버 형식 DB를 가져와 변환할 수 있고, 서버도 모바일 DB를 복원할 수 있습니다. 이 과정에서 신고 row뿐 아니라 `entry_value`, `raw_content`, `synced_at`, 감시목록, `last_sync`, 중복 신고 메타데이터까지 모두 보존하도록 맞춰져 있습니다.
