...
| 탭 | 내용 |
|----|------|
| 대시보드 | 처리 요약, `보완 요청` 카드, 최근 답변, 감시목록, 하단 신고현황(Sunwi) |
| 신고내역 | 교통 / 주정차 / 기타 / 중복차량, `보완요청` 상태/`보완횟수` 검색 |
| 신고관리 | 감시 목록 / 중복 신고 / 데이터 수정 |
| 통계 | 카테고리 요약(총 신고·답변 완료·처리 중·평균 처리기간과 표본 수) + 월별 추이(신고일/답변일 기준) + 연도 × 카테고리 × 유형별 처리 통계, 위반법규 필터 |
...
- 설정 화면에서 앱 테마를 `시스템 설정 사용 / 라이트 모드 / 다크 모드` 중 하나로 직접 고를 수 있습니다.
- 대시보드 요약 카드와 처리 현황 차트에 `보완 요청` 건수가 별도로 표시됩니다.
- standalone/client 공통으로 raw `상태` 와 canonical `처리상태` 를 분리해 저장하므로, `진행`은 원본으로만 남고 검색/대시보드는 `처리중/보완요청/...` 기준으로 동작합니다.
- 상태/처분 배지는 새 디자인 상태색을 씁니다: `보완 요청` 주황, `처리 중` 파랑, `답변 완료` 청록, `수용` 초록, `일부수용` 황색, `불수용/기타` 빨강, `취하` 회색, `과태료` 분홍, `경고/범칙금` 보라. 배지 글자는 라이트/다크 모두 읽기 쉬운 대비(4.5:1 이상)로 자동 보정됩니다. (서버 웹 화면 색은 별도 업데이트 예정이라 당분간 다를 수 있습니다.)
- 대시보드의 `최근 답변 완료 (3일)`에서 `모두 보기`로 들어가면 화면 진입 시 최신 목록을 자동으로 한 번 다시 불러옵니다.
- 중복 신고 알림을 누르면 대표건, child 목록, 상태, 비고까지 포함한 상세 시트를 바로 열 수 있습니다.
- 신고내역 검색 시 처리상태 `보완요청` 과 `보완횟수` 로도 바로 좁혀볼 수 있습니다.
- 개별 신고 카드와 상세 화면에서 **마지막 보완 요청자 / 요청·완료 일시 / 누적 보완횟수**를 함께 확인할 수 있습니다.
...
**Q. 백업 파일은 서로 호환되나요?**  
A. 가능합니다. 모바일 Standalone DB는 서버 형식 DB를 가져와 변환할 수 있고, 서버도 모바일 DB를 복원할 수 있습니다. 이 과정에서 신고 row뿐 아니라 `entry_value`, `raw_content`, `synced_at`, 감시목록, `last_sync`, 중복 신고 메타데이터, 보완요청 관련 컬럼(`보완횟수`, `보완_미응답`, `보완_요청자`, `보완_요청일시`, `보완_완료일시`, `보완_요청_내용`, `보완_신고자_의견`)까지 모두 보존하도록 맞춰져 있습니다.
