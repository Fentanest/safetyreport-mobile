...
상태: **S-01~S-10 결정·구현 완료(2026-09-24)**. 작성 2026-09-24, base `c64be69a`.
...
| 평균 처리기간 | Σ(답변일−신고일) / 유효 표본수 | **유효 표본수**(완료 상태 + 두 날짜 모두 있고 차이 ≥ 0 — S-10) | 두 날짜 | 음수는 제외하고 "날짜 역전 N건" 별도 표시(S-01 결정 필요). 기관 평균의 단순 평균 금지 |
...
avg_days(소수1자리|null, 완료 신고만 — S-10), avg_days_count, reversed_date_count, undated_report_count,
...
`/stats` 행(기관·담당자·법규) 필드: `total avg_days total_fine_amount fine_amount_unknown fines(_pct) warnings(_pct) rejects(_pct) unconfirmed(_pct) in_progress(_pct) avg_rating rating_count`.
`in_progress` 는 2026-09-24 추가(S-10). 행 규칙 테스트: 서버 `test_stats_tables_include_assigned_in_progress_rows` ↔ 모바일 `test/services/stats_tables_test.dart`(같은 입력·기대값).
실데이터 대조(2026-09-24, 5월 DB 사본을 서버 코드와 Standalone import 양쪽에 넣음): 기관·담당자 표 16개 필드와 요약 평균 처리일 전부 일치.
...
| S-07 | **해소(문제 아님)** |
| S-01 | **구현**: Standalone 기관표 평균 처리일도 서버처럼 음수 제외 + 소수 1자리 |
| S-03 | **구현**: 취하 제외 SQL 9곳을 `IFNULL(처리상태,'') != '취하'` 로 — NULL 행 유지(서버 fillna 와 동일) |
| S-04 | **구현**: 대시보드 "처분 미확인" / 통계표 "기타·미분류" (모바일 라벨만, API 필드명 불변) |
| S-05 | **구현**: 행별 `fine_amount_unknown`(서버 `/stats` 행 필드 추가, Standalone `_AgencyAgg`) → 과태료 합계 옆 "금액 미확인 N건" |
| S-06 | **결정(권고대로)**: 현행 유지(중복차량 목록은 대표건 미적용) |
| S-08 | **구현: 답변일** — Standalone 통계·지도 연도 필터와 연도 목록을 답변일로(서버와 동일), 통계 행 drilldown 도 답변일 범위로(Client 기존 불일치도 해소) |
| S-09 | **구현(권고: 완전 일치)**: 서버 `_apply_stats_law_filter` 를 앞뒤 공백 무시 완전 일치로(서버 웹 통계에도 적용). Standalone 은 원래 완전 일치 |
| S-10 | **결정·구현(사용자 2026-09-24)**: 표 포함은 처리상태가 아니라 값으로 — 기관표 = 처리기관 있는 신고, 담당자표 = 처리기관+담당자 있는 신고(`미지정`·빈 값 제외), 처리기관 없으면 어느 표에도 없음('알수없음' 행 폐지). 배정된 처리중도 포함하고 행 필드 `in_progress`/`in_progress_pct`(완료도 취하도 아닌 상태: 처리중·진행·검토중·보완요청·이송·빈 값)로 따로 세며 `unconfirmed` 에서 뺀다. 평균 처리일(행·요약)은 완료 신고만. 행 탭 drilldown 은 그대로(처리중 포함). 화면: 행 카드 '처리중' 배지·막대, '총 처리 N건' → '총 N건'. 서버 웹 표에도 처리중 컬럼. 구서버는 `in_progress` 없음 → 배지 숨김 |
