...
void main() {
  ...
  setUpAll(() async {
    ...
    await databaseFactory.setDatabasesPath(
      Directory.systemTemp.createTempSync('sr_db_photo_test_').path,
    );
  });
  ...
  test('upsertReport keeps photo capture columns when the report is re-saved', () async {
    await LocalDbService.upsertReport(_report(), 'parking', '불법주정차신고-기타 불법주정차');
    final d = await LocalDbService.db;
    await d.update(
      'reports',
      {'사진_첫촬영': '2026-09-22 01:00:00', '사진_끝촬영': '2026-09-22 02:30:00', '사진_촬영수': 3},
      where: 'ID = ?',
      whereArgs: ['p-1'],
    );
    ...
    final row = (await d.query('reports', where: 'ID = ?', whereArgs: ['p-1'])).single;
