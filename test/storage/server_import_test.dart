...
Future<String> _serverDb(Directory dir, {required List<String> watchlist, String? staleMetaWatchlist}) async {
  final path = '${dir.path}/server_${DateTime.now().microsecondsSinceEpoch}.db';
  final db = await openDatabase(path);
  await db.execute('''CREATE TABLE mysafetymerge_traffic (ID TEXT PRIMARY KEY, 상태 TEXT, 신고번호 TEXT, 신고명 TEXT, 신고일 TEXT,
  ...
  for (final (id, number, merged) in [('s1', 'SPP-1', 'Y'), ('s2', 'SPP-2', 'N')]) {
    await db.insert('mysafetymerge_traffic', {
      'ID': id, '상태': '수용', '신고번호': number, '신고명': '신호위반', '신고일': '2026-09-01', '감시목록': merged,
      '처리상태': '수용', '처리기관': '기관', '담당자': null, '위반장소': '서울 강서구 1', '주소정규화': null, '지오코딩상태': null,
      '종결여부': 'Y', 'synced_at': null, '보완횟수': '',
    });
  }
  ...
  if (staleMetaWatchlist != null) {
    await db.insert('mysafety_sync_meta', {'key': 'watchlist', 'value': staleMetaWatchlist});
  }
  await db.close();
  return path;
}
...
void main() {
  ...
  test('empty server watchlist clears flags and ignores a stale sync_meta copy (M-8, S-15)', () async {
    final path = await _serverDb(dir, watchlist: [], staleMetaWatchlist: 'SPP-1');
  ...
  test('values keep NULL and numeric columns get numbers or NULL (M-9, M-10)', () async {
